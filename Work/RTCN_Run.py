...
def train_usertcn(data,options):
    model = tf.keras.models.load_model(f'DL\\rtcn\\best_weight{options[0]} {options[1]} {options[2]}.h5')
...
class newMainWindow(QWidget):
    ...
    def fun_Run(self):
        ...
        if (self.downleft.data_train != None) and (self.downleft.data_val != None) and (len(self.downleft.feature_selected) != 0) and (
            ...
            res = train_usertcn(data, options)
