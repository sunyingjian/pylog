...
def train_usePCA(data,options):
    model = make_pipeline(StandardScaler(),SMOTE(n_components=int(options['n_components']),
                whiten=options['whiten'],
                svd_solver=options['svd_solver']))
    model.fit(data['X_train'],data['y_train'])
    return (model.score(data['X_train']),model.score_samples(data['X_train']))
...
class Downleft(init_ui.downleft):
    ...
    def tab3UI(self):
        ...
        self.combox_whiten.addItem("")
        self.combox_whiten.setCurrentText("False")
        self.combox_whiten.setItemText(0, "False")
        self.combox_whiten.setItemText(1, "True")
        self.label_svd_solver = QLabel("svd_solver:")
        self.label_svd_solver.setEnabled(True)
        self.combox_svd_solvers = QComboBox()
        self.combox_svd_solver.setEnabled(True)
        self.combox_svd_solver.addItem("")        
        self.combox_svd_solver.addItem("")
        self.combox_svd_solver.addItem("")
        self.combox_svd_solvers.addItem("")
        self.combox_svd_solver.setCurrentText("auto")
        self.combox_svd_solver.setItemText(0, "auto")
        self.combox_svd_solver.setItemText(1, "full")
        self.combox_svd_solver.setItemText(2, "arpack")
        self.combox_svd_solver.setItemText(3, "randomized")
...
class newMainWindow(QWidget):
    ...
    def fun_Run(self):
        ...
        if (self.downleft.data_train != None) and (len(self.downleft.feature_selected) != 0) and (
            ...
            whiten = self.downleft.combox_whiten.currentText()
            svd_solver	 = self.downleft.combox_svd_solver.currentText()
            ...
            # 画图
            self.downright.showWidget.setText(str(res)+'聚类文件以保存至data文件夹下')
        print('退出fun_run函数')
